...
the `indentation-consistency` step-tracking, and comment-stripping edge
cases (quoted `#`, mid-token `#`, comment-only lines); `npm test` builds and
runs it with Node's built-in test runner.
